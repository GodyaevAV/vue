...
function eventHandler() {
    ...
    db=getData();
    // console.log(_data);
    // document.getElementById('btnCatalog').addEventListener('click',change_block(_data));
    document.getElementById('btnCatalog').addEventListener('click',change_block);
    ...
    function change_block() {
        document.getElementById('maindiv').classList.add('toggle');
        document.getElementById('catalog').classList.add('toggle');
        document.getElementById('infobar').classList.add('infoBar');
        ...
        for (var i=0;i<base.length; i++) {
            // console.log(base[i]);
            str += "<div class='smallBlock'><img width='90%' height='60%' align='center' src="+base[i].picture+
                "><div class='textjs_div'>"+base[i].name+"</div><p style='" +
                "width: 25%;position: absolute; display: inline-block; right: 20%; top: 60%; font-size: 300%; vertical-align: top;" +
                "'>"+base[i].price+"</p><p class='textYear'>Year: "+base[i].year+"</p><br><input type='button' class='btnBuy' id='"+base[i].index+
                "' value='BUY'></div>";
            // printContent(base[i].name,base[i].price,base[i].year);
        }

        printContent(str,base);
    }

    function createInfoBar(){
        var infoBar = document.getElementById('infobar');
        if(this){
        var prices=(_data[this.id].price);
        prices=prices.substring(1);
        total+=Number(prices);
        schet+=1;}
        infoBar.innerText="";
        var infoBarContent = "<div class='infoBarContent'>" +
            "" +"<div class='infoBarText'>bottles:"+schet+" </div><div class='infoBarText'>total:"+total.toFixed(2)+"</div><div class='btnPay_div'><input type='button' value='pay' class='btnPay'> </div> "
        "</div>";
        infoBar.innerHTML=infoBarContent;
        return 0;
    }

    function printContent(str,base) {
        var aaa = document.getElementById('catalog');
        aaa.innerHTML = str;
        for(var i=0;i<base.length;i++)
        {
            document.getElementById(i).addEventListener('click',createInfoBar);
        }
        createInfoBar();
